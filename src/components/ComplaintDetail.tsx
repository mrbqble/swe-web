...
const ComplaintDetail: React.FC<ComplaintDetailProps> = ({
  ...
  const handleResolve = async () => {
    ...
    try {
      ...
      alert(
        error?.response?.data?.detail || 'Failed to resolve complaint',
      );
    } finally {
      setIsUpdating(false);
    }
  ...
  const handleEscalate = async () => {
    if (
      !window.confirm(
        'Are you sure you want to escalate this complaint to the manager?',
      )
    ) {
      return;
    }
  ...
  const isSalesRep = user?.id === complaint.sales_rep_id.toString();
  const isManager = user?.id === complaint.manager_id.toString();
  const canResolve = permissions.canResolveComplaints && (isSalesRep || isManager);
  const canEscalate = permissions.canEscalateComplaints && isSalesRep && complaint.status === 'open';
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h2 style={{ margin: 0 }}>{t('complaintDetail.title')} #{complaint.id}</h2>
          <button className="btn btn-outline" onClick={onClose} aria-label={t('common.close')}>
            ✕ {t('common.close')}
          </button>
        </div>
        ...
        >
          {complaint.status === 'open' && canEscalate && (
            <button
              className="btn btn-outline"
          ...
          {complaint.status === 'escalated' && isManager && canResolve && (
            <button
              className="btn btn-primary"
          ...
          {complaint.status === 'open' && isSalesRep && canResolve && (
            <button
              className="btn btn-primary"
              onClick={() => setShowResolutionInput(true)}
              disabled={isUpdating}
          ...
          {complaint.status !== 'resolved' && !canResolve && !canEscalate && (
            <div
              style={{
