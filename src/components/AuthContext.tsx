...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	const login = async (email: string, password: string): Promise<boolean> => {
		...
		try {
			...
			// Store tokens (mirrors mobile setTokens helper)
			...
			// On any login failure, ensure we clear any partial auth state
			clearTokens()
			localStorage.removeItem('user')
			setUser(null)
	...
	const signup = async (
		...
		try {
			...
			// On any signup failure, ensure we clear any partial auth state
			clearTokens()
			localStorage.removeItem('user')
			setUser(null)
			// Error is already handled by the caller (Login component)
	...
	return <AuthContext.Provider value={{ user, login, signup, logout, refreshUser, isLoading }}>{children}</AuthContext.Provider>
}
